...
The **Kinetic Action Layer** (`src/back/objects/actions/`) provides a typed, audited mutation path for all domain-object state changes (approvals, overrides, status transitions). Nothing writes to the overlay outside this layer.

**Action Types** are code-registered descriptors that declare a Pydantic params model, validation rules, overlay edits, and outbound effects. The current shipping type is `review_transaction`: an agent proposes an approve/reject decision on a risky transaction; a human accepts it via `approveAction` or overrides it via `overrideAction` (with a mandatory reason). The earlier `flag_customer_high_risk` demo has been retired.

**ActionService** is the single mutation path: validate → lifecycle (AUTO applies immediately; REQUIRES_APPROVAL stays PROPOSED) → overlay upsert + audit record + effect rows in one Lakebase transaction → post-commit `EffectRunner` drain in a background task (best-effort, failures isolated to the outbox row). The new `override()` method marks the agent proposal `OVERRIDDEN` and writes the human's decision overlay in the same transaction.

**Overlay** (`ontology_overlay`, Lakebase Postgres) is the authoritative store for kinetic state. The Triple Store materialises it on next build. **`action_log`** is immutable/append-only; every transition — including the new `OVERRIDDEN` status — is recorded. The **Effect outbox** (`action_effects_outbox`) decouples effect delivery — only `NoopLogEffect` (log the payload) is live; the real moneypool main-app write-back and other connectors are deferred to later slices.

**Entry points**: GraphQL mutations `reviewTransaction`, `approveAction`, `rejectAction`, and `overrideAction` are added to the Mutation type when Lakebase is available. `approveAction` enforces **4-eyes** (`requires_separate_approver=True` on `ReviewTransaction`) — the proposer cannot approve their own action. The agent MCP tool `propose_review_transaction` (`src/agents/tools/actions.py`) lets LLM agents propose decisions. **Overlay read-back is live** — `Transaction.decision` is a `JSON` scalar resolved directly from the Lakebase overlay store. See [Architecture — Kinetic Actions](docs/architecture.md#kinetic-actions) and the design spec under `docs/superpowers/`.
