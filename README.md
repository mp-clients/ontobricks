...
**Action Types** are code-registered descriptors that declare a Pydantic params model, validation rules, overlay edits, and outbound effects. The current shipping type is `review_withdrawal`: an agent proposes an approve/reject decision on a risky withdrawal; a human accepts it via `approveAction` or overrides it via `overrideAction` (with a mandatory reason). The earlier `flag_customer_high_risk` demo has been retired.
...
**Entry points**: GraphQL mutations `reviewWithdrawal`, `approveAction`, `rejectAction`, and `overrideAction` are added to the Mutation type when Lakebase is available. `approveAction` enforces **4-eyes** (`requires_separate_approver=True` on `ReviewWithdrawal`) — the proposer cannot approve their own action. The agent MCP tool `propose_review_withdrawal` (`src/agents/tools/actions.py`) lets LLM agents propose decisions. **Overlay read-back is live** — `Withdrawal.decision` is a `JSON` scalar resolved directly from the Lakebase overlay store. See [Architecture — Kinetic Actions](docs/architecture.md#kinetic-actions) and the design spec under `docs/superpowers/`.
