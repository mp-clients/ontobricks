...
**Entry points**: GraphQL mutations `flagCustomerHighRisk(customerId, severity, reason)`, `approveAction(actionId)`, and `rejectAction(actionId, reason)` are added to the Mutation type when Lakebase is available. `approveAction` enforces **4-eyes per `ActionType`** (`requires_separate_approver`; set on `FlagCustomerHighRisk`) — the proposer cannot approve their own action. `rejectAction` records the reason in the `action_log` audit row. An agent tool (`src/agents/tools/actions.py`) lets LLM agents propose actions. **Overlay read-back fields are live** — each `ActionType` declares `overlay_fields` (property names it writes); the registry aggregates them; `GraphQLSchemaBuilder._add_overlay_fields` attaches a `JSON` scalar (e.g. `Customer.riskFlag`) resolved directly from the Lakebase overlay store. See [Architecture — Kinetic Actions](docs/architecture.md#kinetic-actions) and the design spec under `docs/superpowers/`.
