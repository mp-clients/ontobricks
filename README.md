...
**Entry points**: GraphQL mutation `flagCustomerHighRisk` (added to the Mutation type when Lakebase is available) and an agent tool (`src/agents/tools/actions.py`) so LLM agents can propose actions. **Overlay read-back fields are live** — each `ActionType` declares `overlay_fields` (property names it writes); the registry aggregates them; `GraphQLSchemaBuilder._add_overlay_fields` attaches a `JSON` scalar (e.g. `Customer.riskFlag`) resolved directly from the Lakebase overlay store. See [Architecture — Kinetic Actions](docs/architecture.md#kinetic-actions) and the design spec under `docs/superpowers/`.
